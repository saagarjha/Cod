...
final class CodBasicTests: XCTestCase {
	...
	func testUnkeyedRoundtrip() {
		for testingCodables in Self.combinationsOfTestingCodables {
			TestingUnkeyedContainer.testingCodable = .unkeyed(testingCodables.map {
					.simple($0)
			})
			checkRoundtrip(TestingUnkeyedContainer(values: testingCodables.map {
				$0.random()
			}))
		}
	}
	...
	func testKeyedRoundtrip() {
		for testingCodables in Self.combinationsOfTestingCodables {
			TestingKeyedContainer.testingCodable = .keyed(testingCodables.map {
					.simple($0)
			})
			checkRoundtrip(TestingKeyedContainer(values: testingCodables.map {
				$0.random()
			}))
		}
	}
	...
	func testNestedSingleValueRoundtrip() {
		for testingCodables in Self.combinationsOfTestingCodables {
			TestingSingleValueContainer.testingCodable = .singleValue(.unkeyed(testingCodables.map {
					.simple($0)
			}))
			checkRoundtrip(TestingSingleValueContainer(value: TestingUnkeyedContainer(values: testingCodables.map {
				$0.random()
			})))

			TestingSingleValueContainer.testingCodable = .singleValue(.keyed(testingCodables.map {
					.simple($0)
			}))
			checkRoundtrip(TestingSingleValueContainer(value: TestingKeyedContainer(values: testingCodables.map {
				$0.random()
			})))
		}
	}
