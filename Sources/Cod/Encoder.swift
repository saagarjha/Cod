...
public class CodEncoder {
	...
	fileprivate class UnkeyedContainer: Container, UnkeyedEncodingContainer {
		...
		func encode<T: Encodable>(_ value: T) throws {
			if let value = value as? TransparentlyEncodable {
				...
				values.append(try encode(value))
			}
		}
		...
		func combine() -> Data {
			...
			guard values.count == self.values.count else {
				...
				data.append(contentsOf: BitVector(bits: self.values.map {
					$0 == nil
				}).bytes)
				for value in values {
	...
	fileprivate class KeyedContainer<Key: CodingKey>: Container, KeyedEncodingContainerProtocol {
		...
		func encodeNil(forKey key: Key) throws {
			...
			values[key.stringValue] = .some(nil)
		}
		...
		func encode<T>(_ value: T, forKey key: Key) throws where T: Encodable {
			...
			if let value = value as? TransparentlyEncodable {
				...
				values[key.stringValue] = try encode(value)
			}
		}
		...
		func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> where NestedKey: CodingKey {
			nestedContainer(keyedBy: keyType)
		}
		...
		func combine() -> Data {
			...
			guard values.count == self.values.count else {
				...
				data.append(contentsOf: BitVector(bits: sortedKeys.map {
					self.values[$0]! == nil
				}).bytes)
				for value in values {
					data.append(value.count.uleb128)
...
extension Container {
	...
	func nestedUnkeyedContainer() -> UnkeyedEncodingContainer {
		CodEncoder.UnkeyedContainer(sharedContext: sharedContext)
	}

	func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> where NestedKey: CodingKey {
		KeyedEncodingContainer(CodEncoder.KeyedContainer(sharedContext: sharedContext))
	}

	func superEncoder() -> Encoder {
		CodEncoder.Encoder(sharedContext: sharedContext)
	}
}
